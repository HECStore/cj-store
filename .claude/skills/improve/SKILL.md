...
4. **Execute** the plan: classify each accepted improvement as **inline** (small / one-liner / single-file localized) or **subagent** (large / multi-file / structural). The main agent applies inline edits directly via Read + Edit; subagent improvements are dispatched as fixer subagents, batched into parallel waves when their file sets are provably disjoint and serialized otherwise. (Skipped when dry mode is on.)
...
### 4a. Classify each improvement: inline vs subagent

Before any wave packing, route each accepted improvement to one of two execution paths:

- **Inline** — the main agent applies the edit directly via Read + Edit, no subagent dispatched. Eligible only if BOTH:
  1. `FILES` contains **exactly one** path, AND
  2. the `CHANGE` is a localized, mechanical edit — typo fix, single-occurrence rename, replace-X-with-Y at one site, single-line addition/removal, small wording tweak in a doc/comment, or similar. The net effect is roughly **≤ 3 distinct edited lines**, and the change introduces no new structural block (no new function, no new struct/class, no new test case, no new doc section, no new schema entry, no new module).

- **Subagent** — a fixer subagent is dispatched, exactly as in prior versions of this skill. This is the path for everything that fails either inline criterion: multi-file changes, new tests, new functions, structural refactors, anything that requires reading callers or related files to apply correctly, or any change adding a non-trivial block of new code.

**When in doubt, prefer subagent.** A botched inline edit is more expensive than the cost of dispatching a subagent. Examples that look small but should still go to a subagent: replacing a function body that's more than a few lines; adding "just one" parameter to a function (callers may need updates — even if all in the same file, the surface area exceeds the inline budget); rewording a doc paragraph that runs more than a few lines; deleting a function (the deletion may leave dangling references the main agent should not chase mid-sweep).

Record each improvement's classification before proceeding. The two paths are then handled in 4b–4d below: 4b packs *only the subagent improvements* into waves; 4c applies *only the inline improvements* sequentially in the main agent; 4d dispatches the subagent waves; 4e narrates progress between them.

### 4b. Group subagent improvements for parallel execution

For each **subagent-classified** improvement, you already have its `FILES` list from the proposal. Build a list of `(improvement_id, files_touched)` tuples. Group into **waves** such that within one wave no two improvements share any file path, and any improvement whose files are unclear/unknown is placed in its own singleton wave (serial). Inline-classified improvements are not part of wave packing — they are applied sequentially in 4c, before any wave in their priority bucket fires.
...
5. **Cross-bucket execution order**: process priority buckets sequentially — within each bucket, **inline edits run first** (applied serially in the main agent per 4c), then all subagent Phase A waves, then all Phase B waves; the bucket finishes before the next bucket starts. The full execution chain is therefore P0-inline → P0-A → P0-B → P1-inline → P1-A → P1-B → P2-inline → P2-A → P2-B, skipping any empty slot (a bucket with no inline-classified improvements skips its inline phase; a bucket with no peer-checked subagent improvements skips Phase A; etc.). Each (bucket, phase) slot is its own packing problem — never merge waves across slots, and inline phases are not waves (they are sequential main-agent work and don't consume wave numbers). Wave numbers are continuous across the chain for display purposes only (so the user sees "wave 1, 2, 3, …" without resets between phases or buckets), but the packer must not consider waves outside the current slot when deciding placement.
...
### 4c. Apply inline improvements directly

For each priority bucket, the main agent applies all inline-classified improvements **before** any subagent wave in that bucket fires (per the cross-bucket order in 4b item 5). Walk the inline improvements in their sort order — the same `(tuple, spotter-proposal-number, candidate-kind, adversary-letter, addition-index)` key 4b uses for peer-checked candidates; the same promoted/non-promoted adv-add identity rules carry over. Apply each one sequentially:

1. Use Read to re-read the declared file fresh. **Do not trust line numbers in the proposal text** — earlier inline edits in this same bucket may have shifted lines. Locate the target by symbol/intent, not by line number.
2. Apply the edit via Edit (or Write only when the proposal explicitly creates a brand-new single file from scratch, which is unusual for an inline-classified improvement). Make the minimum edit; preserve surrounding style. Do not refactor adjacent code, do not "tidy up" nearby lines.
3. Stay strictly within the single declared file. If, after reading, the edit turns out to require a second file or a substantial block of new code, STOP, leave the working tree clean for what you did write, mark this improvement internally as `partial` with a `FOLLOWUPS` note describing what the drift sweep should still pick up, and continue. Do NOT silently promote the improvement to a subagent mid-execution; it stays inline-classified for reporting.
4. If the target no longer exists (already fixed, function removed, the issue would be a no-op now), record `obsolete` and continue without editing.
5. If the file changed in a way that makes the edit unsafe and you cannot apply it cleanly, revert any partial edit so the working tree is back to a known-good state, record `blocked`, and continue.
6. After applying, optionally run an obviously-relevant local check the repo uses (the same `cargo check` / `npm run typecheck` convention a fixer would run) on the edited package. Do not run the full test suite. Do not attempt to fix unrelated pre-existing failures.
7. Do not commit.

For each inline improvement, record the same `(STATUS, FILES_EDITED, SUMMARY, FOLLOWUPS)` quadruple a fixer subagent would have produced — these feed into Step 5's drift sweep and Step 6's report on equal footing with subagent fixer output. The `STATUS` legend, the prefer-`partial`-when-in-doubt rule, and the malformed-output bookkeeping all apply here too — the difference is just *who* performs the edit. Inline output cannot be malformed in the subagent sense (the main agent self-reports), but a `blocked` outcome is still possible and is reported normally.

### 4d. Dispatch fixer subagents per wave

For each wave (subagent path only), in a single message send one Agent call per improvement in the wave, all `subagent_type: general-purpose` and all `run_in_background: true`. Wait for the wave to finish before starting the next. (Parallelism within a wave; serial between waves.)
...
### 4e. Between phases and waves

**Before any execution starts**, if the entire run has zero work — zero inline edits AND zero subagent waves to run after the deferral filter (e.g. P0+P1 are empty *and* P2 was deferred) — skip every announcement and say `_Nothing to execute — every accepted improvement was deferred. See "Not landed → Deferred" below._`, then jump directly to Step 6 (Step 5 will skip itself per 5a since no edits landed). The deferred items still appear in the final report.

**Before each bucket's inline phase** (when that bucket has any inline-classified improvements), print a one-liner: `Starting <bucket> inline (M edits).` (substituting `P0` / `P1` / `P2` and the count). Apply the edits sequentially per 4c, then summarize in one line — e.g. `Inline P0: 4 done, 1 obsolete.` — before moving on.

**Before wave 1 of a bucket's subagent execution** (when that bucket has any subagent-classified improvements), print a one-liner: `Starting execution: wave 1 (M improvements).` If the bucket had inline improvements that already ran, this announcement comes after the inline summary so the user sees the transition.

**Between waves**, briefly (one or two lines) tell the user which improvements landed (`done` or `partial`), which were `obsolete` or `blocked`, and which wave is starting next. When the next wave is the first Phase B (adv-add) wave, the first wave of a new priority bucket, or the start of a new bucket's inline phase, mention that explicitly — long sweeps are easier to follow when transitions are flagged. Keep it tight: one line of status + one line of "starting wave N (M improvements)" or "starting <bucket> inline (M edits)".

---

## Step 5 — Drift-reconciliation sweep

After all inline edits and all subagent waves complete (and if dry mode is off), run one final pass to catch drift the individual fixers — and the main agent during inline application — were forbidden from touching. This step is mandatory whenever Step 4 edited at least one file (regardless of whether the edits came from the inline path or the subagent path). It is the ONLY place cross-file drift is allowed to be fixed.

### 5a. Decide whether to run

1. Collect the union of all `FILES_EDITED` paths across every executed improvement — both inline (applied by the main agent per 4c) and subagent (applied by a fixer per 4d) — *after* applying the malformed-output rule (which intersects each subagent fixer's `FILES_EDITED` against its declared `FILES`, drops out-of-scope paths, and zeroes out any internally-inconsistent or unparseable report). The same intersection-against-declared-FILES check applies to inline edits in principle, though the main agent following 4c will not write outside its declared single file. This is the authoritative file set passed to the drift subagent. Separately, gather the text of every non-empty `FOLLOWUPS` field (these are free-text hints, not paths — collected from both inline and subagent reports) and pass them to the drift subagent alongside the authoritative file set as supplementary context. (Out-of-scope paths a rogue fixer wrote to the working tree do *not* enter the drift sweep — they are surfaced under "Malformed" in Step 6 so the user can review and revert manually.)
2. If the authoritative file set is empty (every executed improvement — inline and subagent — was `obsolete`/`blocked` with no edits), skip this step and go to Step 6 — without any new source of truth there is nothing for the drift sweep to reconcile against, even if some `FOLLOWUPS` were reported. (Surface those `FOLLOWUPS` in Step 6's "Follow-ups worth considering" section instead.)
...
A table: one row per improvement whose execution **returned a parseable status** (`done`, `partial`, `obsolete`, or `blocked`) — both inline edits applied by the main agent and subagent fixer reports. Columns: `#`, `Priority`, `Mode`, `Target`, `Status`, `Files edited`. `Mode` is `inline` (main agent applied directly per 4c) or `subagent` (fixer dispatched per 4d). Subagent fixers whose output was malformed are NOT listed here — they appear only in "Not landed → Malformed" below; inline edits cannot be malformed in the subagent sense (the main agent self-reports), but a `blocked` outcome is still possible and is listed here normally.
...
- **Obsolete / blocked**: an executed improvement (inline or subagent) couldn't apply the change. Quote the reason from its `SUMMARY`.
- **Deferred**: P2 items intentionally not executed (neither inline-applied nor subagent-dispatched) because including them would have pushed the union of `FILES` paths across P0+P1+P2 above the 10-distinct-paths threshold (per Step 4). These can be picked up by re-running `/improve`.
- **Malformed**: any subagent (spotter, adversary, fixer subagent, drift) whose output couldn't be parsed and was treated as empty per the malformed-output rule. List one line per dropped subagent — role and target/improvement only, no reproduction of the garbled output. (Inline edits cannot be malformed since the main agent self-reports; an inline edit that couldn't be applied lands under "Obsolete / blocked" instead.)
...
- **Spotters return 1+ proposals.** Each spotter's report is a list — one big change, several small ones, or a mix, sized to whatever the sampled target area actually warrants. Quality over quantity; don't pad to a number, don't stop at one if more genuinely deserve calling out. Each proposal must stand alone (a fixer is spawned per accepted proposal — inline by the main agent or via subagent — and subagent fixers may execute in parallel).
...
- **Dry mode is a flag** toggled by the literal token `dry` anywhere in the args. When on, stop after Step 3 (the plan is printed; no inline edits are applied, no fixer subagents are dispatched, no drift sweep runs). When off, execute the plan.
- **Adversaries read the target themselves** — they don't just argue from the original's text.
- **Spotters and adversaries don't edit files.** They only propose. Editing happens in Step 4 — small/one-liner edits applied inline by the main agent (4c), larger/multi-file edits dispatched to fixer subagents (4d).
- **Inline edits in the main agent for small / one-liner improvements; subagent fixers for everything else.** Step 4a classifies each accepted improvement: a single declared file plus a localized mechanical change (~≤ 3 edited lines, no new structural blocks) is applied inline by the main agent; everything else is dispatched as a fixer subagent. When in doubt, prefer the subagent path — a botched inline edit is more expensive than the cost of dispatching a subagent. Inline improvements are sequential and run before subagent waves within their priority bucket; the same STATUS / FILES_EDITED / SUMMARY / FOLLOWUPS reporting and the same drift-sweep handoff applies in both paths.
- **Fixers stay in scope (subagent and inline alike).** Each improvement is implemented exactly as proposed, with no drive-by cleanup or refactoring of nearby lines — whether the edit is applied by a fixer subagent or by the main agent inline. Scope creep is the single most common way these sweeps go wrong.
- **Parallel only when provably disjoint.** If two improvements' file sets overlap, or if a file set is unknown/uncertain, the improvements go in different waves. Conservative wins; a wrong parallelization corrupts someone's edits. (Inline improvements are not packed into waves — they run sequentially in the main agent — so this rule constrains only the subagent path.)
...
- **Same-tuple siblings run in numeric order *within their classification path*.** Within a single spotter's tuple, two inline-classified siblings run in numeric order during their bucket's inline phase, and two subagent-classified siblings preserve numeric order across waves (PROPOSAL N is never in an earlier wave than PROPOSAL N-1). When siblings *span paths* — one is inline-classified and the other subagent-classified — the inline phase always runs first per the bucket-phase order in 4b item 5, so the inline sibling lands before the subagent sibling regardless of their numeric order. This is acceptable because the spotter prompt requires every proposal to be robust to its siblings not having landed; cross-path mixing only ever causes a proposal to see *more* of its siblings already in place, never less.
- **Fixers locate by symbol, not by line (subagent and inline alike).** Earlier edits in the same sweep may have shifted lines, renamed symbols, or moved code. Every fixer subagent — and the main agent when applying an inline edit per 4c — re-reads its declared FILES fresh and locates its target by symbol/intent, not by the line numbers the proposal text mentioned.
