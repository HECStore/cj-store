...
> reads of financial data (balance lookups AND trade-log queries
> against another player's UUID) are doubly gated: `tools_store_enabled`
> AND `tools_store_cross_player_balance_lookups` (the latter mirrors
> `cross_player_reads`; financial state is more sensitive than the
> per-player memory bullets and chat-history whisper records that
> `cross_player_reads` covers, so it gets its own switch). `query_trades` defaults to
> self-scope when the model omits `user_uuid`, and per-trade output
> drops the `user_uuid` field unless the cross-player flag is on.
> Operator status is **never** exposed
...
- `mpsc::Sender<HistoryItem>` (capacity 4096) — Bot → history-writer task,
  ...
  in `chat::history` with three variants: `Inbound(ChatEvent)` for
  observed chat lines, `BotOut { … }` for lines the bot itself emitted
  (chat or whisper), and `DropCacheAndAck { ack }` — a control message
  used by `scrub_history_for_player` to drop the writer's cached
  `LineWriter<File>` (so the next event re-opens by path) and ack back,
  fencing GDPR rewrites against the live writer cache. **Publishers use
  `try_send`, never `await`** — a hostile flooder must not be able to
  block `bot_task` by filling this channel.
...
| `read_player_memory`   | `uuid` OR `username`                    | **Cross-player firewall**: returns `"access denied"` unless `resolved_uuid == current_event.sender.uuid`. Operator can flip `chat.cross_player_reads = true` for trusted single-tenant servers. The auth check fires BEFORE any `mojang::resolve_user_uuid` call: by-`uuid` denies on UUID mismatch, and by-`username` consults the local player index first — only a self-name match (or `cross_player_reads = true`) falls through to Mojang. This seals the username-existence oracle and saves Mojang rate budget on denied paths. |
...
| `read_today_history`   | `since_event_ts?`, `limit_lines?`       | Returns today's JSONL, capped at `chat.tools_history_max_bytes` (default 32 KB ≈ 8 K input tokens). Pagination via `since_event_ts`. Most recent first. **Cross-player firewall**: when `chat.cross_player_reads` is OFF (default), records are restricted to ones where the caller is `sender` (by username), or `target_uuid` equals the caller's UUID, or `kind` starts with `bot_` (the bot's own output). Whisper records targeting other players are filtered out. |
| `search_history`       | `query`, `days_back`                    | Substring search **scoped strictly to `data/chat/history/*.jsonl`**. Returns up to 50 matches, each capped at 1 KB. Streaming line scan via `tokio::task::spawn_blocking`. **Cross-player firewall**: same scope as `read_today_history` — when `chat.cross_player_reads` is OFF (default), only sender-or-target lines and `bot_*` kinds are returned. |
...
| `get_user_balance`     | `uuid` XOR `username`                   | **Store data, opt-in.** Reads `data/users/<uuid>.json` via [`store_view::user::UserView`](src/chat/store_view/user.rs) — the `operator` field is **deliberately not deserialized** (so the chat surface cannot leak operator status). The cross-player auth check fires BEFORE any `mojang::resolve_user_uuid` call: by-`uuid` denies on UUID mismatch; by-`username` consults the local player index first, and only a self-name match (or `tools_store_cross_player_balance_lookups = true`) falls through to Mojang. This seals the username-existence oracle and saves Mojang rate budget on denied paths. Cross-player lookups gated by `tools_store_cross_player_balance_lookups` (default false; mirrors `cross_player_reads`). Balance may be up to `autosave_interval_secs` stale. |
...
| `Chat: toggle dry-run`                           | Runtime override of `chat.dry_run`. Persists in `state.json`. Refused with a "pinned ON by `config.dry_run`" message when `chat.dry_run = true` in config — the runtime override would be silently overridden by the effective composition (`config.dry_run \|\| runtime_override`), so the operator must edit config and restart to truly disable dry-run. |
...
| Leaking cross-player data       | Cross-player firewall on `read_player_memory` AND on the history tools (`read_today_history` / `search_history` filter whisper records targeting other players when `cross_player_reads` is off) + Trust-gated proactive references; Trust 3 is the only level allowed to reference cross-player history.                    |
...
| Prompt injection in chat        | Nonce-tagged `<untrusted_chat_…>` wrappers (CSPRNG-drawn 12-hex-char nonce); any literal `<untrusted` or `</untrusted` substring inside the wrapped content (any case) is neutralized to `<_untrusted` / `<_/untrusted` before wrapping; oversized content is truncated to 4 KB on a UTF-8 boundary with a `…[truncated]` sentinel. |
