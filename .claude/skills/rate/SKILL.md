...
A table with one row per **surviving** original target (any tuple dropped for parse failure or hallucinated-target does not appear in the table — it is summarized in the `_Excluded:_` line(s) below the table instead). Each row has a `#` index, a `Target` label (one-line description plus file path), the `Original` rating, one rating column per adversary (Adv A through Adv {Kth letter} — exactly K columns when K ≥ 1, none when K = 0), and a `Tuple mean` column. Empty cells (e.g., a single dropped adversary) are rendered as `—`. When K = 0, there are no adversary columns and the tuple mean equals the original rating.
...
Below the table, render one `_Excluded: <count> — <reason>_` line per distinct exclusion reason that occurred (e.g., one line for parse-failure drops, a separate line for hallucinated-target drops). Omit the row entirely when no exclusions occurred.
...
(Section heading shown above is the literal heading to render when improve mode is on. The "improve mode only" qualifier is documentation, not part of the rendered title.) Include this section **only when improve mode is on**; otherwise omit it entirely.
...
If no bullets remain after exclusions, skips, and the null-improvement filter, render the section as a single line — "_No actionable improvements surfaced in this sample._" — and stop.
...
- **High priority** — fixes multiple agents converged on (across tuples or within a tuple), or correctness/security issues (not style); additionally, any **non-stylistic** fix sourced from a tuple that meets the disagreement threshold defined in "Notable disagreements" above — i.e., when K ≥ 1, |adversary − original| ≥ 0.15 for any adversary in the tuple; when K ≥ 2, additionally max − min ≥ 0.15 across all surviving ratings in the tuple. This applies to every threshold-meeting tuple, regardless of whether it made the displayed top 3-5. The disagreement signals a contentious area worth acting on. Stylistic/cosmetic fixes from a contentious tuple stay at low priority — disagreement is not enough to elevate them. When K = 0 there is no adversary signal, so only the "multiple agents converged" and "correctness/security" sub-rules apply.
